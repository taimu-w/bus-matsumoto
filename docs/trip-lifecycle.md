...
（旧・条件②「終了エリア到達」（直近GPSが終点から一定距離以内）は、循環線・往復線の途中で誤発火する不具合（点検所見 H-1）があり撤去しました。）

**2段階到着判定（[pass-detection.md](pass-detection.md)）導入後、終点は`付近`のまま条件①が成立しないまま止まり続けることがあります。** 終点は到着後にバスがそのまま停車し続けることが多く、`DEPARTURE_MARGIN_METERS`分だけ離れる（＝到着確定のトリガー）が起きないためです。この場合は条件④（GPS途絶）が成立した際に、`endAssignment()`が`state='ended'`にする直前に`付近`のまま残っている終点を記録済みの最小距離の観測時刻で強制的に`到着済`へ昇格させます（`interpolated=FALSE`）。終点が`付近`まで来ていればその観測値を優先して昇格させます（`''`のままなら従来どおり直近の生GPS時刻へフォールバックし`interpolated=TRUE`）。
