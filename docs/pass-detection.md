...
- **循環線対策④（同名バス停の通過順序ゲート、`computeSameNameOrderGate()`）**: 往復非分離の循環線（美ヶ原温泉線・浅間線など）は、同じ物理バス停を1便で2回通る。往路の標柱と復路の標柱はGTFS上おおむね別`stop_id`（同名で十数〜百数十m隣り合う。まれに完全に同一`stop_id`）で、`trip_stop_progress`には`seq_order`違いの**同名複数行**として現れ、その2点は`STOP_RADIUS_METERS`（120m）圏内に収まるほど近い。**同名のより手前のバス停がまだ`到着済`でないうちは、その先の同名行（2回目以降の通過）を付近入り候補から外す**（バスは同じ場所を1回目→2回目の順にしか通れない）。手前の同名がすべて`到着済`になれば従来どおり候補に戻る。1回しか通らない通常のバス停・同名の1回目・手前が片付いた2回目はゲートに入らない。
  - **防いでいる誤り**: 1回目が`到着済`になる前にGPSのばらつきで2回目の行へ最近傍マッチすると、2回目の行に1回目相当の早い時刻が入り、`promoteStuckNearbyStops()`／線形補間で確定して「**2回目のバス停通過時刻が1回目の通過時刻になる**」（美ヶ原温泉線の新井橋など、実データで確認済み）。
  - 始発バス停は`openAssignment()`で`到着済`になるため、始発と同名の終点（松本バスターミナル等）はゲートされない。
