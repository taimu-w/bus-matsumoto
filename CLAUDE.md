...
素のHTML/CSS/JS、ビルドステップなし。`frontend/index.html` + `app.js`（利用者向け運行状況画面。20秒間隔で`/api/buses`等をポーリング、お気に入りはlocalStorage、SPAルーティングの入口）。`frontend/timetable.js`（時刻表検索）、`frontend/busstop.js`（バス停検索）、`frontend/stopmap.js`（バス停マップ）、`frontend/routesearch.js`（経路検索）、`frontend/spotsearch.js`（スポット検索。地名・バス停・路線1つから、スポット情報＋付近のバス停＋周辺路線を表示。路線チップでリアルタイム時刻表`#/realtime/...`へ）はいずれもハッシュではなくパスでルーティングします。観光スポットの写真表示（バス停ページの周辺観光スポット・経路検索のスポット詳細ポップアップ・スポット検索のスポット情報カード）は`frontend/spot-photos.js`（`window.SpotPhotos`、`busstop.js`等より先に読み込む）に一本化し、1枚ずつ表示するカルーセル（複数枚は5秒間隔の自動送り＋スワイプ／矢印／インジケーター。自動送りは1本の共有タイマー＋レジストリで回し各tickで`isConnected`を見て漏れを掃除）を描画します。各画面は`SpotPhotos.markup(spot, opts)`でHTMLを組み立て、DOM挿入後に`SpotPhotos.hydrate(scope)`を呼ぶだけです（[docs/tourist-spots.md](docs/tourist-spots.md)の「写真表示（カルーセル）」）。`frontend/admin.html`（サーバー側セッション（httpOnly Cookie）で保護された、PC向けサイドパネル型の運行監視コンソール。運行ダッシュボード・車両運用状況・便の割当監視・予測精度の監視・当日の状況（路線別のペース補正状況・遅延メッシュ地図）・異常アラート・GTFS/位置情報フィード監視・API稼働監視・ジョブ監視・お知らせ編集・バス停お知らせ・祝日カレンダー・外部IDマッピング・方向マッピング・リアルタイム休止・運用パラメータ設定・観光スポット管理・観光スポットの検索・アクセス数（スポット検索の検索回数`spot_search_counts`と公式サイトリンクのタップ回数`tourist_spot_link_clicks`を指定期間でまとめて集計）・車両名・メモ管理・サイト閲覧数・運行実績ダウンロード）。運行ダッシュボード（`admin-dashboard.js`）はバスアイコンをタップすると右パネルにリアルタイム時刻表（定刻＋予測 or 実績）を表示し、各バス停の行をタップすると詳細モーダルで「到着済＝判定方法（付近経由／ベクトル判定／手動 等）と根拠＋遅れ」「未到着＝ETA予測根拠（`source`＋ペース補正の内訳）」を、いずれもETA予測の推移（`trip_arrival_prediction_log`）を折れ線グラフ（横軸＝予測時刻・縦軸＝予測到着時刻・定刻は破線）＋「変化の記録」リストで表示します。このモーダルは15秒ポーリングで更新されますが、レスポンスが前回と同一なら再描画せず、変化があってもスクロール位置を維持します。到着判定時刻を空にして保存するとそのバス停を未到着に戻せます。「異常アラート」の`gpsLostTrip`（GPS途絶で便打ち切り＝`trip_vehicle_assignments.end_reason='GPS更新停止'`）は「地図で検証」ボタンから全画面モーダル（`admin-gps-outage.js`、`GET /api/admin/gps-outage/:assignmentId`）を開き、運行ダッシュボードと同じ地図で「担当開始以降の走行軌跡」「途絶地点（赤）／復旧地点（緑）と継続分数」「途絶時点で時刻表のどこまで進んでいたか」を表示します（走行軌跡は`GPS_LOG_RETENTION_HOURS`＝既定48時間を過ぎると空）。便詳細セクションの車両表示は、管理画面「車両名・メモ管理」（`GET/PUT/DELETE /api/admin/vehicle-labels`、`vehicle_labels`テーブル、キーは`car_id`）で名前を付けた車両は`car_id`ではなく名前で表示し、車両名（名前が無ければ車両ID）タップで車両詳細（直近の運行履歴＝直近の平日1日分・土休日1日分の全便の運行日/路線名/行先/始発時刻、およびメモ）を表示します（`GET /api/admin/vehicle-operation-history/:carId`、`vehicle_operation_history`テーブル）。同じ内容はサイドバー「車両運用状況」（`admin-vehicle-operation-status.js`、`GET /api/admin/vehicle-operation-status`）で全車両ぶん一覧できます。外部IDマッピング・方向マッピングの編集セクションはいずれも路線を`/api/routes`の候補一覧から選ぶ方式で、路線名の自由入力による表記ゆれ事故が起きない設計です。方向マッピングは「方向で絞り込まない（ignore）」と「変換表で判定（map）」を切り替え、mapのときCSV方向値→`direction_id`の変換表とフォールバックを編集します（`admin-direction-rules.js`）。「バス停お知らせ」（`admin-busstop-notices.js`、`busstop_notices`テーブル、`services/busstopNotices.js`）は見出し＋画像＋本文（リンク記法対応・ただの文章も可。画像と本文の少なくとも一方）を1件に組み合わせられるお知らせで、バス停詳細ページ（`busstop.js`）の「このバス停でできること」の下に出します。配信範囲が2つあり、`scope='stop'`（バス停単位）は表示モードによらず常に、`scope='platform'`（乗り場単位）は**乗り場が確定しているとき（乗り場別表示 or 乗り場が1か所）だけ**表示します（統合表示中やお知らせが無い範囲はカードごと出しません）。GTFSインデックスとは乗り場単位なら`feed_id`+`stop_id`、バス停単位なら`stop_key`（＋別名）だけで結びつけ（`gtfsTimetable.resolvePlatformRef()`で解決）、`tourist_spots`と同じく保存時に固い外部キーを張りません。詳細は[docs/busstop-notices.md](docs/busstop-notices.md)。

### セキュリティ設定（`config/security.js` / `middleware/` / `services/adminAuth.js`）

管理画面の認証・HTTPS強制・セキュリティヘッダー・CORS・レートリミットは、**運用パラメータ（管理画面「運用パラメータ設定」）とは別系統**で、`backend/src/config/security.js`が環境変数だけから解決します（管理画面から編集できません。レートリミットやセッション有効期限を管理画面から変えられると、設定ミスがそのまま管理画面自身へのロックアウトになるため）。既定値はすべて「設定しなければ従来どおり動く」側に倒してあります。一覧はREADME §8。

- 管理API（`/api/admin/*`）の認証は`services/adminAuth.js`が一手に引き受け、**サーバー側セッション（httpOnly Cookie）とBasic認証ヘッダーの2経路**を通します。管理画面はセッション、curl・監視ツールはBasic。判定の入口は`api.js`の`isAuthenticatedAdmin()`のままです。
- セッションはプロセス内メモリなので、再起動・デプロイで失効します（＝再ログイン）。管理画面は401を受けると`handleSessionExpired()`でログイン画面へ戻ります。
- レートリミットは`middleware/rateLimit.js`（依存追加なしのインメモリ固定ウィンドウ）。掛けているのは`/api/route-search`と集計値を増やす2本、および管理画面の認証**失敗**回数だけで、20秒ポーリングされるホットパス（`/api/buses`等）には掛けていません。
- Tailwind・Leafletは`frontend/vendor/`に同梱しており、CDNからは読み込みません（[frontend/vendor/README.md](frontend/vendor/README.md)）。
...
- **`frontend/index.html`の静的ファイル参照は必ず絶対パス（`/app.js`・`/style.css`・`/vendor/...`）にしてください。** 時刻表検索は`/timetable/stops/{stop_id}`のような階層のあるURLを使うため、相対パスだとブラウザが`/timetable/stops/app.js`を取りに行き、サーバーのSPAフォールバックがindex.htmlを返してスクリプトが一切動かなくなります。
- **管理画面の資格情報をブラウザ側（localStorage / sessionStorage）に保存する実装へ戻さないでください。** 以前は`btoa("user:pass")`をlocalStorageに置いていましたが、base64は暗号化ではなくXSSが1件でもあれば資格情報ごと抜かれ、共用端末では次の利用者にも残ります。現在は`POST /api/admin/session`で発行するhttpOnly Cookie（JSから読めない）で認証し、資格情報をネットワークに載せるのはログインの1回だけです。`api()`にAuthorizationヘッダーを足す必要はありません（同一オリジンなのでCookieが自動で付きます）。
- **`requireAdminAuth`が認証失敗を数えるのは「資格情報を提示して外したとき」（＝Basic認証ヘッダーがあるとき）だけです。** 期限切れのセッションCookieを失敗として数えないでください。数えると、セッションが切れた管理画面のポーリング（3つのタイマー）が数十秒で上限に達し、**管理者が自分自身をロックアウトします**。判定は`adminAuth.hasPresentedCredentials()`に集約してあります。
- **`frontend/vendor/leaflet/images/`を消さないでください。** `leaflet.css`が`url(images/marker-icon.png)`と相対参照しており、Leafletの既定マーカーもこのCSSからパスを検出します。同じ階層に`images/`が無いと既定マーカーとレイヤー切替アイコンが表示されなくなります。Tailwindはファイル名にバージョンが入るため、更新時は参照している4つのHTML（`index.html`・`admin.html`・`howto.html`・`servicestatus.html`）を全部書き換えること（1つでも古いままだとそのページだけ別バージョンで描画されます）。
...
- **`route_external_ids`・`route_direction_rules`・`config/feeds.js`の3つは「位置情報とGTFSを結びつける設定」で似ていますが別物です。混同しないでください。** `route_external_ids`＝外部ID⇔route_id（DB・管理画面）、`route_direction_rules`＝CSV方向値⇔direction_id（DB・管理画面・既定ignore）、`config/feeds.js`＝フィードURL/有効無効/位置情報フィード⇔GTFSフィードの対応（コード・要デプロイ）。詳細は[docs/feed-config.md](docs/feed-config.md)。`route_realtime_suspensions`（管理画面「リアルタイム休止」）はこれらとは目的が別で、「位置情報⇔GTFSの結合設定」ではなく**公開画面のリアルタイム表示のキルスイッチ**です。
- **リアルタイム休止（`route_realtime_suspensions`・`services/realtimeSuspension.js`）は`services/realtimeTripLookup.js`の`findLiveAssignment()`1か所のガードで公開面をまとめて止めます。** この関数の外部呼び出し元（経路検索の重ね合わせ・便詳細の「リアルタイムに切替」・バス停の「接近中のバス」）はいずれも`null`をsoft-failするため、休止路線に`null`を返すだけで済みます。`/api/buses`・`/api/buses-for-map`は`findLiveAssignment()`を経由しない独自クエリなので個別に休止判定を持ちます。**管理画面の運行監視（運行ダッシュボードの地図・便の割当監視・異常アラート）は意図的に対象外**で、`assignment_id`直引きの別経路（`getAssignmentDetailForAdmin()`等）を通るためこのガードにかからず、休止中の路線も従来どおり全便を監視できます。`/api/buses-for-map`だけは公開エンドポイントなので、認証済みの管理画面リクエスト（`isAuthenticatedAdmin(req)`）のときだけ除外をスキップします。解除は行の削除のみ（自動解除なし）。詳細は[docs/realtime-suspension.md](docs/realtime-suspension.md)。
