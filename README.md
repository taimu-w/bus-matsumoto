...
**フィード構成と対応はコード（`backend/src/config/feeds.js`）で管理し、`feeds`テーブルは稼働状態（`last_fetched_at`/`last_status`/`last_error`）の記録のみを担います。** 外部ID⇔GTFS route_idの対応も同様にコード（`backend/src/config/routeExternalIdMapping.js`）です。設計背景は[docs/外部IDマッピングのコード化_仕様書.md](docs/外部IDマッピングのコード化_仕様書.md)を参照してください。フィード構成・外部ID対応の追加・変更はコード側を編集してデプロイします（管理画面からは編集できません）。
