...
const SETTINGS_CATALOG = [
  ...
  // ---- 深夜運行停止時間帯 ----
  {
    ...
    default: '23:00',
    label: '深夜運行停止の開始時刻',
    description: 'この時刻からNIGHT_ENDまでの間、位置情報取得・車両割り当て・通過判定・遅延計算・到着予測を停止します（当日便の生成のみ継続します）。ただし始発時刻を過ぎても割り当てが済んでいない便が残っている間は、安全弁としてこれらの処理も継続します。HH:mm形式で指定してください。'
  },
  {
    key: 'NIGHT_END',
    group: 'night',
    ...
    default: '05:00',
    label: '深夜運行停止の終了時刻',
    description: '深夜運行停止が終わる時刻。最も早い便は5:40発のため、これを5:40より遅くすると早朝便の車両割り当てが停止時間帯に入ります（その場合も未割り当ての便が残っている間は安全弁として処理を継続しますが、意図しない設定でないか確認してください）。HH:mm形式で指定してください。'
  },
